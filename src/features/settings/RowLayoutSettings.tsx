...
import { RowsAccordionIcon } from "../../icons";
...
export function RowSettingsContent() {
  const { activePattern, updateRowSpec, removeRow, toggleRowOnRiser, setRowShift, addRow, selectedEditRow, setSelectedEditRow } = useApp();
      ...
      {!validSelection ? (
        ...
        <>
          <p className="rowsEditorNote">
            {segmentsTotal(rows[selectedEditRow].segments)}人 の列を編集中
          </p>
          <RowEditorPanel
            key={selectedEditRow}
          ...
          <button
            type="button"
            className="btn danger rowDeleteBtn"
            onClick={() => { removeRow(selectedEditRow); setSelectedEditRow(null); }}
          >
            この列を削除
          </button>
        </>
