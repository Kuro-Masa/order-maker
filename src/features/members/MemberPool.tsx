...
function parseMemberCsv(text: string): Member[] {
  ...
  for (const raw of lines) {
    ...
    if (!name || name === "氏名") continue; // skip header
    members.push({
...
function ImportDialog({ onClose }: { onClose: () => void }) {
  ...
  function handleFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      if (parsed.length === 0) {
        setError("メンバーが見つかりませんでした。CSVの形式を確認してください。");
        pendingRef.current = [];
      } else {
  ...
  function handleImport() {
    if (pendingRef.current.length === 0) {
      setError("先にCSVファイルを選択してください。");
      return;
    }
    setMembers(pendingRef.current);
    onClose();
  }

  return (
    <div className="memberDialogBackdrop" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="memberDialog" role="dialog" aria-modal="true" aria-label="CSVでメンバーを取り込む">
        <h2 className="memberDialogTitle">CSVでメンバーを取り込む</h2>
        <p className="memberDialogDesc">
          パート1・パート2・氏名の3列で作成したCSVを取り込みます。
        </p>
        <div className="memberDialogCols">
          列: <code>パート1</code>&nbsp;<code>パート2</code>&nbsp;<code>氏名</code>
          <br />
          例: <code>Sop</code>&nbsp;<code>1st</code>&nbsp;<code>田中 花子</code>
        </div>
